...
## 52. The configuration and the record store, brought up to the guard

Date: 2026-09-16

Entry 51 recorded thirty eight changes that the tests accepted in `internal/config` and `internal/state`, and did not make them. This is those. Both are at zero and both are in the continuous integration job, which now covers six packages: two hundred and fifty three changes, two hundred and thirty four noticed, none accepted.

In the configuration package, twelve. Most were boundaries that the range checks turn on and nothing had ever touched: that a percentage of exactly 1 and exactly 100 are inside the range, that two context thresholds set to the same value are refused because a warning and a confirmation at the same reading means one of them never happens, and that a session setting of exactly 1 is a value rather than an absence. Three were whole functions that could be inverted with nothing disagreeing: `Exists`, which every command asks before doing anything, `Step`, which `verify` and `ship` ask before running anything, and `Changed`, which decides whether `policy migrate` writes. A section written as `null` in the file was the last one, and it is now reported rather than walked into.

In the record store, twenty six. The ones worth naming: a record from a schema version this build does not know was read as though it were current, and a file the store does not own could be read as a record. Both are now told apart by a fixture that is valid session JSON under a name the store did not write, which is the only way to tell the two halves of that rule apart. The baseline for the rate projection keeps the lowest reading of the window that is running, and a reading equal to it must not move the moment the baseline was taken, because the rate is measured over the span since that moment and moving it would quietly halve the rate. And only an ask waits to be approved, which a denial must not.
...
## 55. Handing work to another engine, tested before building

Date: 2026-09-17

The proposal was that when a session hits its limit, ZeroTurn should take what the session knows and hand it to a local open model such as Ollama or DeepSeek. Stop conditions were written down first, before anything was looked at, the same way the two ideas in entry 38 were killed. They are kept in the session scratchpad and are reported against by name below.

The first thing worth settling is what can be shared at all. There are three things people mean by context and only one of them moves. Weights do not: different architectures and different tokenizers. The working state of a conversation does not either, and not fixably, because it is a function of that model's own weights and there is no general format to put it in. What is left is text and files. The repository is already shared, because it is on disk. The conversation is not, and the only way to move it is to write it down. So every version of this idea reduces to producing a document.

K2 failed. Several tools already do this, and they are not obscure. Each one works by reading the transcript of the session, which is the one thing this project is built never to do and tests in both directions. So ZeroTurn cannot compete on their terms, and the version worth considering is a different artifact rather than a better one: built from what is observable, with the notes written deliberately by the agent rather than extracted from what it said, and bound to a repository state and a validation result that can be checked. Nobody else's handoff can say that what it describes was true of exactly this code.

K3 was met, with numbers. The Handoff Debt study interrupted agents at fixed points across seventy five tasks, giving one hundred and eighty one handoff points and seven hundred and twenty four takeover runs for each successor model. Against a takeover given only the repository, which is exactly the position somebody is in without this, a handoff carrying context cut the median number of agent events by twenty to fifty nine percent and cumulative prompt tokens by forty two to sixty three percent.

The line after that is the one to keep. Solved rates improved less, and by model, while the efficiency gains held across all of them. A handoff makes taking over cheaper. It does not make it more correct, and saying otherwise would be the kind of overstatement this project exists to avoid. The same study separates deterministic checkpoint state from notes the previous model wrote, which is the split arrived at here independently and is the reason ZeroTurn has anything to add: it owns the half that can be verified.

K1 survived, but not as it was framed. ZeroTurn extracts nothing. The agent writes what it decided and what it assumed, because it is the only party that knows; ZeroTurn contributes the repository snapshot, the validation evidence and the Git state, and binds them together. Observed and asserted are kept apart in the record for the same reason result and current are kept apart in evidence: one of them can be checked and the other cannot, and a reader has to be able to tell which is which.

K4 was not reached, because a record that is written and handed over by a person needs no model call, no model selection and no SDK. K5 stands as a limit rather than a kill: session pressure reaches ZeroTurn only through the status line, so a trigger based on it works in a terminal and nowhere else, and the trigger has to be a command or a change in repository state.

The finding that decides the original proposal is about direction. The Handoff Tax study separates handing up to a stronger model from handing down to a weaker one, and the effect is asymmetric: giving less of the previous trajectory improved quality when escalating, and removing the strong model's trajectory hurt quality when downshifting. Paid to local is a downshift. It is the direction that depends most on the trajectory, and smaller models were found to take disproportionately larger penalties on takeover. So the proposal names the hardest of the two directions and the one that most needs the thing ZeroTurn is forbidden to read.
